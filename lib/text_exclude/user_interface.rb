...
module TextExclude
  ...
  class UserInterface
    ...
    def user_pattern
      ...
      search_history.each_pair do |index, pattern|
        if pattern == '' # wait for next open slot
          $search_history[index.to_s] = @pattern.to_s # store it for TextProcessor class
          break
        end
      end
      @pattern
    ...
    # Any need adjustmest to the display are done based on the controls
    # "before" and "after"
    def user_display(text_area)
      @text_area = text_area # save for user_write
      puts '======== ====5====1====5====2====5====3====5====4====5====5' \
           '====5====6====5====7====5====8====5====9====5====0====5====1' \
           '====5====2====5=='
      text_area.each do |line, action|
        if action[0] == 'before'
          puts '-------- ----------------------------------------------' \
               "---------- #{action[1]} lines excluded "\
               '---------------------------------------------------'
        end
        if action[0] == 'text'
          format = format('%08d %-80s', line, action[1])
          puts format
        end
        if action[0] == 'after'
          puts '-------- ----------------------------------------------' \
    ...
    # This is the start of the UX where we ask for a file or the
    # selection of a file for input. The real value of something like
    # this in a command line interfaces is to be able to save to disk.
    # Of course that is easy to do but since I never really work in the
    # command line I forgot. Thus we start out here with a standard tree
    # directory (for the time being only Linux). This is for the input
    # file but it is the same for output.
    def user_file_read
      selection = ''
      file_information = {}
    ...
    # List the files from a directory with an index
    # number so it can be selected
    ...
    # File output starts in the directory of the current file. The
    # assumption is one will want to keep things together or over write
    # the current file.
    ...
    # Everything up until this point has been assumed to be an
    # exclude request. These are the fundamental options
    ...
    #  6. Write to file
    #  7. Exit
    def user_prompt_options(_text_area)
      puts <<-DELIMITER
      1. Additional include pattern
      2. Additional exclude pattern
      3. Delete all included text
    ...
    def user_prompt_ranges(text_area, text_lines)
      ...
      if $mode == 'test'
        ...
        ARGF.each_line do |selection_range|
          ...
          break if (0..9).cover?(@selection_split[0].to_i) # index reused from above
        end
      end
      ...
      @selection_split.each do |s|          # check for range indicated
        ...
        next unless s.include?('..') # range given?
        range_split = s.split('..') # yes, split it for first and last
        @selection_split[1] = range_split[0]
      ...
      if @selection_split.length < 3 # check length entered
        puts 'Format is: Selection number then Range (11..23) ' \
             'or Amount with "after" number'
