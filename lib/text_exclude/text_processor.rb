...
module TextExclude
  # TextProcessor is responsible for all of the functions performed
  ...
  class TextProcessor
    ...
    def text_include(text_lines)
      ...
      text_lines.each do |line_number, text| # read the file line by line
        ...
        $search_history.to_h.each_pair do |_symbol, pattern| # get the current search patterns
          @pattern = pattern unless pattern == '' # formal argument cannot be an instance variable
          unless pattern == '' || !/#{Regexp.escape(pattern)}/.match(text)
            @found = true
          end
        end

        if @found                       # is it what is being looked for?
          if @exclude_count.positive?   # yes, none excluded yet?
          ...
          unless @line_number == @text_end # line position
            @text_area.store(@line_number + 1, ['fill', ''])
          end
    ...
    def text_exclude(text_area)
      ...
      $search_history.to_h.each_pair do |_symbol, pattern| # get possible last search pattern
        if pattern == ''
          break # past last one
        else
    ...
    def text_delete_in(text_lines)
      line_number  = 0
      text_area    = {}
      text_lines.each do |_line_num, text| # read the file line by line
        found = false

        $search_history.to_h.each_pair do |_symbol, pattern| # get the current search patterns
          found = true unless pattern == '' || !/#{Regexp.escape(pattern)}/.match(text)
        end
    ...
    # The big picture is there are two things in play, the text_area
    # which is built as the first exclude display and the text_lines
    # which is the original input file used to rebuild the text_area as
    # required. Beyond that the requirement is to track the various
    # "lines excluded" positions, the number of lines excluded for
    # various conditions related to the actions required in rebuilding
    # the original exclude display. In particular block start is the
    # exclude line information after the "after" line when doing any
    # action. The block end is needed for adjustments to the display for
    # the exclude line after the last line in the action range. While
    # the block start information is for the exclude after the beginning
    # of the action range the block prior is needed for rebuilding the
    # display with actions requiring additional excludes. Clearly if the
    # prior exclude ends on the line before the additional requested
    # excludes starts the prior excludes need to be joined with the
    # current.
    #
    def text_mixer(text_area, parameters)
      ...
      #
      # find the section of data to modify based on finding the
      # "before" type preceding and following the range
      # it is also necessary to keep track of the block prior
      # to the block preceding for uses with excludes of excludes
      #
      ...
      @keys.each do |k| # find prior text_area of type "before"
        next unless k <= @line_start # use this to format new excluded data
    ...
    def text_mixer_include(_parameters)
      @text_area.each do |ta|
        @new_text_area.store(ta[0], ta[1]) if ta[0] < @line_start
      end
      @new_text_area.store(@line_start - 1,
                           ['before', (@block_start_index - @line_start) - 1])
      text_write_lines(@line_start - 1, @line_end - 1)
      @new_text_area.store(@block_end_index,
                           ['before', @block_end_index - @line_end])
      @text_area.each do |ta|
        @new_text_area.store(ta[0], ta[1]) if ta[0] > @block_end_index
      end
      @new_text_area
    end
    ...
    def text_mixer_exclude(_parameters)
      @new_text_area.store(0, ['text', 'Not yet implemented'])
      @new_text_area
   end

    # 3. Delete included lines in range.
    # Request is for a range of lines to be deleted if included
    # in the display.
    ...
    # Request is for a range of lines to be deleted if excluded from
    # the display
    def text_mixer_range_exclude
      ...
      @text_area.each do |ta|
        ...
        if (@line_start..@line_end).cover?(ta[0])           # in range?
          ta1 = ta[1]                                       # get [type, data]
          @new_text_area.store(ta[0], ta[1]) if ta1[0] == 'text'
        end

        if (@line_end...@block_end_index).cover?(ta[0])
    ...
    def text_mixer_range_insert(parameters)
      ...
      @text_lines.each do |tl|
        if @line_start == tl[0] + 2
          ...
          @line_start.upto(@line_end) do
            @text_area.store(@text_lines_index += 1, 'insert')
          end
        else
          @text_area.store(@text_lines_index += 1, tl[1])
        end
    ...
    # The text area is the "memory" map used to build the display
    # logged to console.
    # This gets called out of the various formatter to write a
    # line from memory to the display.
    # Format is typically: [23, ["fill", ""]] or [24, ["before", 13]]
    def text_write_area(from, to)
      @text_area.each do |ta|
        @new_text_area.store(ta[0], ta[1]) if (from..to).cover?(ta[0])
      end
    end

    # The text lines area is the copy of the original file read
    # in from disk.
    # All this does is write from the text lines area to the display
    # when requested
    # by formatters that are restoring excluded lines.
    # Format is: [25, ["text", "25 r s t u v rstuv line2"]]
    def text_write_lines(from, to)
      @text_lines.each do |tl|
        if (from..to).cover?(tl[0])
          @new_text_area.store(tl[0] + 1, ['text', tl[1]])
        end
      end
    end
  end
